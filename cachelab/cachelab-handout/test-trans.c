...
#define LOG_LIMIT 95 * 1024 * 1024

static char log_buf[LOG_LIMIT + 100];
...
void eval_perf(unsigned int s, unsigned int E, unsigned int b) {
    ...
    for (i = 0; i < func_counter; i++) {
        ...
        sprintf(cmd,
                ...
                "./tracegen -M %d -N %d -F %d",
                M, N, i);

        full_trace_fp = popen(cmd, "r");
        char* p = log_buf;
        int log_cnt = 0;
        flag = 0;
        ...
        while (fgets(buf, 1000, full_trace_fp) != NULL) {
            ...
            if (buf[0] == ' ' && buf[2] == ' ' &&
                ...
                int cnt = sprintf(p, "%s", buf);
                if ((log_cnt += cnt) > LOG_LIMIT) {
                    printf(
                        "Too many memory operations (store/load/modify)! "
                        "Please check your code to remove redundant memory "
                        "operations OR there is something wrong with your code "
                        "that memory misses are too large to count!\n");
                    flag = 1;
                    break;
                }
                p = p + cnt;
        ...
        if (flag) {
            pclose(full_trace_fp);
            continue;
        }

        flag = WEXITSTATUS(pclose(full_trace_fp));
        if (0 != flag) {
            printf(
        ...
        flag = 0;
        memory_fp = fmemopen(log_buf, log_cnt, "r");

        while (fgets(buf, 1000, memory_fp) != NULL) {
        ...
        char cmd[255];
        sprintf(cmd, "./csim-ref -s %u -E %u -b %u -t %s > /dev/null", s, E, b,
                filename);
        system(cmd);
...
void usage(char* argv[]) {
    ...
    printf("  -t          Used in autolab testing.\n");
...
int main(int argc, char* argv[]) {
    ...
    while ((c = getopt(argc, argv, "M:N:ht")) != -1) {
        switch (c) {
            case 'M':
                M = atoi(optarg);
                break;
