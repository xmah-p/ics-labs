 ...
 */
#define PERLPROG \
    "while(<>){chomp; s/\\s+//g; s/\\(\\d+\\)/\\(PID\\)/g; print \"$_\"}"
...
int main(int argc, char** argv) {
    ...
    while ((c = getopt(argc, argv, "Ai:t:s:hVx")) != EOF) {
        switch (c) {
            ...
            case 'V': /* Increase verbosity level */ verbose++; break;

            case 'x': /* Enable sandboxing (hidden flag used by autograder) */
                sandboxing = 1;
                break;

            case 'h': /* Print help */ usage(); exit(0);
    ...
    sprintf(test_raw_outfile, "/tmp/test_raw_outfile.%d.%d", current_time, pid);
    sprintf(ref_raw_outfile, "/tmp/ref_raw_outfile.%d.%d", current_time, pid);
    sprintf(diff_raw_outfile, "/tmp/diff_raw_outfile.%d.%d", current_time, pid);

    sprintf(test_filtered_outfile, "/tmp/test_filtered_outfile.%d.%d",
            current_time, pid);
    sprintf(ref_filtered_outfile, "/tmp/ref_filtered_outfile.%d.%d",
            current_time, pid);
    sprintf(diff_filtered_outfile, "/tmp/diff_filtered_outfile.%d.%d",
            current_time, pid);
...
int runtrace(char* tracefile) {
    ...
    sprintf(buf, "perl -e '%s' < %s | sort > %s", PERLPROG, test_raw_outfile,
            test_filtered_outfile);
    system(buf);

    sprintf(buf, "perl -e '%s' < %s | sort > %s", PERLPROG, ref_raw_outfile,
    ...
    if (status != 0) {
        sprintf(buf, "diff %s %s > %s\n", test_raw_outfile, ref_raw_outfile,
                diff_raw_outfile);
        system(buf);
...
void delete_tmpfiles() {
    char buf[MAXBUF << 3];
    sprintf(buf, "rm -rf %s %s %s %s %s %s", test_raw_outfile, ref_raw_outfile,
            diff_raw_outfile, test_filtered_outfile, ref_filtered_outfile,
