...
int main(int argc, char** argv) {
    ...
    while ((c = getopt(argc, argv, "hVxs:f:")) != EOF) {
        switch (c) {
            ...
            case 'V': /* Be more verbose */ verbose++; break;
            case 's': /* The shell program name (default ./tsh) */
                shellprog = strdup(optarg);
                break;
            case 'f': /* Trace file name */ tracefile = strdup(optarg); break;
            case 'x':           /* Enable sandboxing */
                sandboxing = 1; /* Hidden argument */
                break;
            default: usage("Unrecognized argument");
        }
    }

    if (!tracefile) usage("Missing required argument (-f)");
    ...
    while (fgets(line, MAXBUF, tracefp)) {
        ...
        /* Ignore blank lines */
        if (blankline(line)) {
            if (verbose) printf("runtrace: Ignoring blank line\n");
            continue;
        }
        ...
        if (verbose) printf("runtrace: command=%s line=%s\n", command, line);
        ...
        if (!strcmp(command, "WAIT")) {
            if (readable(syncfd[0], DRIVER_TIMEOUT) == 0) {
                ...
                if (verbose) printf("runtrace: received sync from job\n");
                continue;
            }
        }
        ...
        /* SIGNAL command */
            ...
            if (verbose) printf("runtrace: sent sync to shell job\n");
            continue;
        }
...
void clean() {
    system(
        "/bin/kill -9 tsh tshref mytstpp mytstps mycat myenv myintp myints "
        "myspin1 myspin2 mysplit > /dev/null 2>&1");
}
