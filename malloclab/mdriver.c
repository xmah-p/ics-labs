...
int main(int argc, char** argv) {
    ...
    double secs, ops, util, avg_mm_util, avg_mm_throughput = 0, p1, p2,
                                         perfindex;
    ...
    while ((c = getopt(argc, argv, "d:f:c:s:t:v:hpVAlD")) != EOF) {
        switch (c) {
            ...
            case 'V': /* Increase verbosity level */ verbose += 1; break;

            case 'v': /* Set verbosity level */ verbose = atoi(optarg); break;

            case 'd': debug_mode = atoi(optarg); break;

            case 'D': debug_mode = DBG_EXPENSIVE; break;

            case 's': set_timeout = atoi(optarg); break;
    ...
    if (run_libc) {
        printf(
            "Comparison with libc malloc: mm/libc = %.0f Kops / %.0f Kops = "
            "%.2f\n",
    ...
    if (autograder) {
        sprintf(
            autoresult,
            "{\"scores\": {\"Autograded Score\": %.0f}, \"scoreboard\": [%.0f, %.0f, %.0f, %0.f\
]}",
...
static trace_t* read_trace(stats_t* stats, const char* tracedir,
    ...
    if (trace->ignore_ranges != 0 && trace->ignore_ranges != 1) {
        app_error("%s: ignore-ranges can only be zero or one",
                  trace->filename);
    }
...
static void reinit_trace(trace_t* trace) {
    ...
    memset(trace->block_sizes, 0,
           trace->num_ids * sizeof(*trace->block_sizes));
    ...
    for (i = 0; i < trace->num_ops; i++) {
        ...
        switch (trace->ops[i].type) {
            case ALLOC: /* mm_malloc */
                 ...
                 * properly, and must not overlap any currently allocated
                 * block.
                 */
...
static void eval_mm_speed(void* ptr) {
    ...
    for (i = 0; i < trace->num_ops; i++) switch (trace->ops[i].type) {
            ...
            default: app_error("Nonexistent request type in eval_mm_speed");
        }
}
...
static void printresults(int n, stats_t* stats, sum_stats_t* sumstats) {
    ...
    for (i = 0; i < n; i++) {
        if (stats[i].valid) {
            switch (stats[i].weight) {
                case WNONE: wstr = ' '; break;
                case WALL: wstr = '*'; break;
                case WUTIL: wstr = 'u'; break;
                case WPERF: wstr = 'p'; break;
                default: app_error("wrong value for weight found!");
            }
    ...
    if (errors == 0) {
        ...
        printf("%2d %2d  %5.0f%%%8.0f%10.6f%6.0f\n", sum_util_weight,
               sum_perf_weight, util, sumops, sumsecs, tput);
...
static void usage(void) {
    ...
    fprintf(stderr,
            "\t-c <file>  Run trace file <file> once, check for correctness "
            "only.\n");
